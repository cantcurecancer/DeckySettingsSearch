...
- `OptionBare.dc.html` — the same board under its option name, so the option keeps its identity.
...
Every board is 300 by 700 px, the real column width. The dark ground behind the panel (#0e141b) is
...
the design skill (`/design`), passing all five boards and `canvas.json`. The assembled page is
